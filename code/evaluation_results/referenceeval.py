...
def compare(relevance_csv, predictions_csv, language):
    relevance_scores = load_relevances(relevance_csv)
    predictions = load_predictions(predictions_csv)
    # Now Compute the various evaluation results
    a = coverage_per_language(predictions[language], relevance_scores[language]) * 100
    b = coverage_per_language(predictions[language], relevance_scores[language], with_positive_relevance=True) * 100
    c = ndcg(predictions[language], relevance_scores[language])
    d = ndcg(predictions[language], relevance_scores[language], ignore_rank_of_non_annotated_urls=False)
    return a, b, c, d
...
if __name__ == '__main__':
    args = docopt(__doc__)
    run(args)
    compare(args['RELEVANCE_ANNOTATIONS_CSV_PATH'], args['MODEL_PREDICTIONS_CSV'], 'python')
